...
function theme_add_scripts() {
    ...
    wp_enqueue_script('jquery-migrate', get_template_directory_uri() .'/js/jquery-migrate.min.js', array(), '1.0', true );
    ...
    wp_enqueue_script('slick', get_template_directory_uri() .'/slick/slick.min.js', array(), '1.0', true );
...
class Kama_Breadcrumbs {
	...
	static $args = [
		...
		'title_patt'      => '<span class="breadcrumbs__link breadcrumbs__link--active">%s</span>',
	...
	function get_crumbs( $sep, $l10n, $args ) {
		...
		$arg->sep = '<span class="breadcrumbs__link__separator">' . $arg->sep . '</span>'; // supplement
		...
		if( 1 ){
			...
			if( ! $mark ){
				$mark = [
					'wrappatt'  => '<div class="kama_breadcrumbs breadcrumbs__link">%s</div>',
			...
			elseif( $mark === 'rdf.data-vocabulary.org' ){
				$mark = [
					'wrappatt'  => '<div class="kama_breadcrumbs breadcrumbs__link" prefix="v: http://rdf.data-vocabulary.org/#">%s</div>',
			...
			elseif( $mark === 'schema.org' ){
				$mark = [
					'wrappatt'  => '<div class="kama_breadcrumbs breadcrumbs__link" itemscope itemtype="http://schema.org/BreadcrumbList">%s</div>',
					'linkpatt'  => '<span class="breadcrumbs__link" itemprop="itemListElement" itemscope itemtype="http://schema.org/ListItem"><a href="%s" class="breadcrumbs__link" itemprop="item"><span itemprop="name">%s</span></a></span>',
					'sep_after' => '',
				];
			}
